...
for rj in glob.glob(str(OUT / "*/report.json")):
    ...
    # SCA divergence: each pkg-vuln finding carrying a CVE -> one set per scanner
    for f in r.get("findings", []):
        if f.get("category") == "pkg-vuln" and f.get("scanner") in SCA:
            ...
            if cid and str(cid).startswith("CVE"):
                sca_sets[f["scanner"]].add((img, cid))   # CVE level: fair across the engines' different package-naming schemes
    rows.append({
